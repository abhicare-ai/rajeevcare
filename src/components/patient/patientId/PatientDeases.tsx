...
export default function PatientDeases({ patinetId }: PatientDeasesProps) {
  const {
    data,
    fetchNextPage,
    hasNextPage,
    ...
    queryKey: ["appointmet-feed", "patient-deases", patinetId],
    queryFn: ({ pageParam }) =>
      kyInstance
        .get(
          `/api/patient/${patinetId}/deases`,
  ...
  return (
    ...
    >
      {posts.map((post) => (
        <DeasesList key={post.id} deasesList={post}/>
      ))}
